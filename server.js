...
// Initialize GLM-5.2 on startup (optional, never crashes)
(async () => {
  try {
    ...
    console.log('[GLM-5.2] Init skipped:', err.message);
    glm52Status = { connected: false, error: err.message };
  }
})();

// Initialize AionUi on startup (optional, never crashes)
(async () => {
  try {
    ...
    console.log('[AionUi] Init skipped:', err.message);
    aionuiStatus = { enabled: false, error: err.message };
  }
})();
...
// Model Switcher Routes
app.get('/api/models/status', (req, res) => {
  res.json(modelSwitcher.getStatus());
});

app.get('/api/models/providers', (req, res) => {
  res.json({
    free: modelSwitcher.getFreeProviders(),
    paid: modelSwitcher.getPaidProviders(),
    current: modelSwitcher.getStatus()
  });
});

app.post('/api/models/switch', (req, res) => {
  const { provider, model } = req.body;
  const result = modelSwitcher.switchProvider(provider, model);
  if (result.success) {
    modelSwitcherStatus = modelSwitcher.getStatus();
    logEvent('system', 'model_switched', `Switched to ${result.name}`);
  }
  res.json(result);
});

app.post('/api/models/chat', async (req, res) => {
  const { agent, messages, options = {} } = req.body;
  if (!messages) return res.status(400).json({ error: 'messages required' });

  const result = await modelSwitcher.chat(agent || 'commander', messages, options);
  res.json(result);
});

// GitHub Fix (uses current model)
app.post('/api/models/fix-file', async (req, res) => {
  ...
  // Simplified version — full version is in executePlan
  res.json({ success: false, error: 'Use the main command API with "fix file ..."' });
});

// Legacy routes (redirect to new endpoints)
app.get('/api/openrouter/status', (req, res) => {
  res.json({ note: 'Use /api/models/status', data: modelSwitcher.getStatus() });
});
